...
class AoCDropdownView(discord.ui.View):
    ...
    def __init__(
        ...
        day_and_star_data: dict[str:dict],
        maximum_scorers: int,
    ):
        super().__init__()
        self.day = 0
    ...
    def generate_output(self) -> str:
        ...
        header = AOC_DAY_AND_STAR_TEMPLATE.format(
            rank="Rank", name="Name", completion_time="Completion time (UTC)"
        )
        lines = [f"{header}\n{'-' * (len(header) + 2)}"]

        for rank, scorer in enumerate(
            self.data[f"{self.day}-{self.star}"][: self.maximum_scorers]
        ):
            time_data = datetime.fromtimestamp(scorer["completion_time"]).strftime(
                "%I:%M:%S %p"
            )
            lines.append(
                AOC_DAY_AND_STAR_TEMPLATE.format(
                    datastamp="",
                    rank=rank + 1,
    ...
    async def fetch(
        ...
        if self.day == 0 or self.star == 0:
            await interaction.response.send_message(
                "You have to select a value from both of the dropdowns!", ephemeral=True
            )
        else:
