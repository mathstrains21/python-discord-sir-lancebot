...
BASE_HEADERS = {"Client-ID": CLIENT_ID, "Accept": "application/json"}
...
class Games(Cog):
    ...
    async def renew_access_token(self) -> None:
        ...
        while True:
            ...
            logger.info(
                f"Successfully renewed access token. Refreshing again in {time_delta}"
            )
    ...
    async def _get_genres(self) -> None:
        ...
        async with self.http_session.post(
            f"{BASE_URL}/genres", data=body, headers=self.headers
        ) as resp:
            result = await resp.json()
    ...
    async def games(
        self, ctx: Context, amount: Optional[int] = 5, *, genre: Optional[str]
    ) -> None:
        """
        ...
        if not 1 <= amount <= 25:
            await ctx.send(
                "Your provided amount is out of range. Our minimum is 1 and maximum 25."
            )
            return
        ...
        try:
            games = await self.get_games_list(
                amount, self.genres[genre], offset=random.randint(0, 150)
            )
        except KeyError:
        ...
        await ImagePaginator.paginate(
            pages, ctx, Embed(title=f"Random {genre.title()} Games")
        )
    ...
    async def genres(self, ctx: Context) -> None:
        ...
        await ctx.send(
            f"Currently available genres: {', '.join(f'`{genre}`' for genre in self.genres)}"
        )
    ...
    async def search(self, ctx: Context, *, search_term: str) -> None:
        ...
        await LinePaginator.paginate(
            lines, ctx, Embed(title=f"Game Search Results: {search_term}"), empty=False
        )
    ...
    async def company(self, ctx: Context, amount: int = 5) -> None:
        ...
        companies = await self.get_companies_list(
            limit=amount, offset=random.randint(0, 150)
        )
    ...
    async def get_games_list(
        ...
        async with self.http_session.post(
            url=f"{BASE_URL}/games", data=body, headers=self.headers
        ) as resp:
    ...
    async def create_page(self, data: dict[str, Any]) -> tuple[str, str]:
        ...
        url = COVER_URL.format(
            **{"image_id": data["cover"]["image_id"] if "cover" in data else ""}
        )

        # Get release date separately with checking
        release_date = (
            dt.utcfromtimestamp(data["first_release_date"]).date()
        ...
        companies = (
            [c["company"]["name"] for c in data["involved_companies"]]
        ...
        formatting = {
            ...
            "rating_count": data["total_rating_count"]
            if "total_rating_count" in data
            else "?",
            "platforms": ", ".join(platform["name"] for platform in data["platforms"])
    ...
    async def search_games(self, search_term: str) -> list[str]:
        ...
        for game in data:
            formatting = {
                ...
                "url": game["url"],
                "rating": round(
                    game["total_rating"] if "total_rating" in game else 0, 2
                ),
    ...
    async def get_companies_list(
        ...
        body = COMPANIES_LIST_BODY.format(**{"limit": limit, "offset": offset})

        async with self.http_session.post(
            url=f"{BASE_URL}/companies", data=body, headers=self.headers
        ) as resp:
            return await resp.json()
    ...
    async def create_company_page(self, data: dict[str, Any]) -> tuple[str, str]:
        ...
        url = LOGO_URL.format(
            **{"image_id": data["logo"]["image_id"] if "logo" in data else ""}
        )

        # Try to get found date of company
        founded = (
            dt.utcfromtimestamp(data["start_date"]).date()
        ...
        developed = (
            ", ".join(game["name"] for game in data["developed"])
            if "developed" in data
            else "?"
        )
        published = (
            ", ".join(game["name"] for game in data["published"])
