...
LOVE_DATA = json.loads(
    Path("bot/resources/holidays/valentines/love_matches.json").read_text("utf8")
)
...
class LoveCalculator(Cog):
    ...
    async def love(
        self, ctx: commands.Context, who: Member, whom: Optional[Member] = None
    ) -> None:
        """
        ...
        """
        if Lovefest.role_id not in [role.id for role in who.roles] or (
            whom is not None
            and Lovefest.role_id not in [role.id for role in whom.roles]
        ):
            raise BadArgument(
        ...
        embed.set_footer(
            text=f"You can unsubscribe from lovefest by using {Client.prefix}lovefest unsub"
        )
