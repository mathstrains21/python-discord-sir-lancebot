    ...
    *allowed_months: Month, sleep_time: Union[float, int] = ONE_DAY
    ...
    def decorator(task_body: Callable) -> Callable:
        ...
        async def decorated_task(*args, **kwargs) -> None:
            ...
            log.info(
                f"Starting seasonal task {task_body.__qualname__} ({human_months(allowed_months)})"
            )
            ...
            while True:
                ...
                if current_month in allowed_months:
                    ...
                    log.debug(
                        f"Seasonal task {task_body.__qualname__} sleeps in {current_month!s}"
                    )
    ...
    def decorator(listener: Callable) -> Callable:
        ...
        async def guarded_listener(*args, **kwargs) -> None:
            ...
            if current_month in allowed_months:
                ...
                log.debug(
                    f"Guarded {listener.__qualname__} from invoking in {current_month!s}"
                )
    ...
    async def predicate(ctx: Context) -> bool:
        ...
        for role in ctx.author.roles:
            if role.id in role_ids:
                log.debug(
                    f"{ctx.author} has the '{role.name}' role, and passes the check."
                )
                return True
...
def whitelist_check(**default_kwargs: Container[int]) -> Callable[[Context], bool]:
    ...
    def predicate(ctx: Context) -> bool:
        ...
        log.trace(
            f"Calling whitelist check for {ctx.author} for command {ctx.command.name}."
        )
...
def whitelist_override(
    bypass_defaults: bool = False, **kwargs: Container[int]
) -> Callable:
    """
